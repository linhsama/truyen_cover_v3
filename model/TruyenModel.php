...
class TruyenModel extends Database
{
    ...
    public function Truyen__Get_Per_Paged_Nhom_Truyen($page_number, $nhom_truyen_id)
    {
        ...
        $obj = $this->connect->prepare(
            "SELECT 
            truyen.truyen_id,
            MAX(truyen.truyen_ten) AS truyen_ten,
            MAX(chapter.chapter_so) AS latest_chapter_so,
            MAX(chapter.chapter_id) AS latest_chapter_id,
            MAX(truyen.truyen_anh_bia) AS truyen_anh_bia,
            MAX(truyen.truyen_luot_xem) AS truyen_luot_xem,
            MAX(truyen.truyen_ngay_dang) AS truyen_ngay_dang,
            MAX(chapter.chapter_ngay_cap_nhat) AS max_ngay_cap_nhat
            FROM (truyen
    ...
    public function Truyen__Get_Top_Views_Paged($page_number)
    {
        ...
        $obj = $this->connect->prepare(
            "SELECT 
                truyen.truyen_id,
                MAX(truyen.truyen_ten) AS truyen_ten,
                MAX(chapter.chapter_so) AS chapter_so,
                MAX(chapter.chapter_id) AS chapter_id,
                MAX(truyen.truyen_anh_bia) AS truyen_anh_bia,
                MAX(truyen.truyen_luot_xem) AS truyen_luot_xem,
                MAX(truyen.truyen_ngay_dang) AS truyen_ngay_dang,
                MAX(chapter.chapter_ngay_cap_nhat) AS max_ngay_cap_nhat
